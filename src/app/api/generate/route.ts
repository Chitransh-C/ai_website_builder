...
7.  All JavaScript must be self-contained in the 'js' key,Check the user's prompt for any mention of external libraries (like Three.js, D3.js, GSAP, etc.). If a library is mentioned, you MUST add its public CDN URL to the "external_scripts" array.IF YOU can use jQuery ,Anime.js, AOS and others.  If no external libraries are needed, return an empty array [].
